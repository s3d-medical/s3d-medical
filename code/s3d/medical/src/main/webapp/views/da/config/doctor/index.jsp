...
$(function(){
	...
	BUI.use(['bui/grid','bui/data'],function(Grid,Data){
		  ...
		  renderer:BUI.Grid.Format.enumRenderer(fdStatusEnumObj)
			...
			renderer : function(value,obj){
				var retArr = [];
					...
					retArr.push(aobj.join('<c:url value="/da/config/doctors/edit/" />'+value));
					var aobj = ['<a href=',' target=_blank><i class="icon-close" title="删除"></i></a>'];
					retArr.push(aobj.join('<c:url value="/da/config/doctors/delete/" />'+value));
				return retArr.join('&nbsp;&nbsp;&nbsp;&nbsp;');
			}
		}];
		...
		tbar.items.push({
	        ...
	        text:'<i class="icon-plus"></i>新建医生',
	        handler : function(){
	        	var openurl = '<c:url value="/da/config/doctors/add" />';
		 ...
		 */
		/*var store = new Store({
			url : '<c:url value="/da/config/doctors" />',
		...
		function changeHospital () {
			...
			/*store.set('url',store.get('url') + '?hospitalId=' + hospitalId);
            store.load();*/
		}
	});
	...
	<div class="container">
		...
		<p>
			在此你可以对<span class="label label-important"><strong>医生</strong></span>进行编辑!
		</p>
		</div>
